...
void MonteCarlo(vector<plaquette4> &plaq, vector<site4> &lattice, int n, int nt, double &beta, double dbeta, int iterations, int Wilson_lenght, int number_file, int &numberWilson){
  ...
  minimize4_energy(plaq, plaquettes_size,lattice, lattice_size, n, nt, beta, iterations, fileE, fileP, numberWilson, Wilson_lenght);
...
int main(){
  ...
  string choose;
  cout<<"Would you like a random initial lattice (R) o the freeze one (F) ?"<<endl;
  cin>>choose;
  

  vector<site4> lattice;
  vector<plaquette4> plaq;
  int lattice_size;

  if(choose=="R"){
      lattice4Random_creation(lattice, n, nt);
      lattice_size=lattice.size();
      plaquette4Random_creation(plaq, lattice, lattice_size, n, nt);
  }

  else{
      lattice4_creation(lattice, n, nt);
      lattice_size=lattice.size();
      plaquette4_creation(plaq, lattice, lattice_size, n, nt);
  }
  ...
  while(beta<beta1){
    MonteCarlo(plaq, lattice, n, nt, beta, dbeta1, iterations, Wilson_lenght, number_file, numberWilson);
    number_file++;
  }
  ...
  while(beta<beta2){
    MonteCarlo(plaq, lattice, n, nt, beta, dbeta2, iterationshigh, Wilson_lenght, number_file, numberWilson);
    number_file++;
  }
  ...
  while(beta<=beta_max){
    MonteCarlo(plaq, lattice, n, nt, beta, dbeta3, iterations, Wilson_lenght, number_file, numberWilson);
    number_file++;
  }
